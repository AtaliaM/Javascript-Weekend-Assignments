function returnNTribonacciElements (startFrom = 1, N) {
    let firstNElements = [startFrom, startFrom, 1];

    for (let i = 3; i <= N; i++) {
        firstNElements.push(firstNElements[i-3]+firstNElements[i-2]+firstNElements[i-1]);
    }
...
returnNTribonacciElements(1, 10);
returnNTribonacciElements(0, 10);
...
// if n == 0, then return an empty array (except in C return NULL) and be ready for anything else 
